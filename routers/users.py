...
from schema import ShowResponse, Token, UserCreate, UserPrivate, UserPublic, UserUpdate
...
@router.get("/{user_id}/shows", response_model=list[ShowResponse])
async def get_user_shows(user_id: int, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    result = await db.execute(
        ...
        .where(models.Show.user_id == user_id)
...
async def update_user(user_id: int, user_update: UserUpdate, current_user: CurrentUser, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    if user_update.username is not None and user_update.username.lower() != user.username.lower():
        result = await db.execute(select(models.User).where(func.lower(models.User.username) == func.lower(user_update.username)))
        existing_user = result.scalars().first()
        if existing_user:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if user_update.email is not None and user_update.email.lower() != user.email.lower():
        result = await   db.execute(select(models.User).where(func.lower(models.User.email) == func.lower(user_update.email)))
        existing_email = result.scalars().first()
        if existing_email:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
